...
public class XPath2Panel extends JPanel {
    ...
    private JSyntaxTextArea expectedValue;
    ...
    private void init() { // WARNING: called from ctor so must not be overridden (i.e. must be private or
        ...
        topBox.add(makeXpathExpressionPanel());
        topBox.add(Box.createVerticalGlue());
        topBox.add(Box.createVerticalGlue());
        topBox.add(Box.createVerticalGlue());
        topBox.add(Box.createVerticalGlue());
        topBox.add(makeExpectedValuePanel());
    ...
    private JPanel makeXpathExpressionPanel() {
        JPanel panel = new JPanel(new GridBagLayout());
        GridBagConstraints gbc = new GridBagConstraints();
        initConstraints(gbc);
        panel.add(new JLabel("Xpath Expression:  "), gbc.clone());
        gbc.gridx++;
        gbc.weightx = 1;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        panel.add(JTextScrollPane.getInstance(getXPathField(), true), gbc.clone());
        resetContraints(gbc);
        return panel;
    }
    
    private JPanel makeExpectedValuePanel() {
        JPanel panel = new JPanel(new GridBagLayout());
        GridBagConstraints gbc = new GridBagConstraints();
        initConstraints(gbc);
        panel.add(new JLabel("Expected Value:"), gbc.clone());
        gbc.gridx++;
        gbc.weightx = 1;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        panel.add(JTextScrollPane.getInstance(getExpectedValueField(), true), gbc.clone());
        resetContraints(gbc);
        return panel;
    }
    ...
    public String getExpectedValue() {
    	return this.expectedValue.getText();
    }
    ...
    public void setExpectedValue(String expectedValue) {
    	this.expectedValue.setInitialText(expectedValue);
    }
    ...
    public JSyntaxTextArea getXPathField() {
        if (xpath == null) {
            xpath = JSyntaxTextArea.getInstance(20, 50);
            xpath.setLanguage("xpath"); //$NON-NLS-1$
        }
        return xpath;
    }
    
    public JSyntaxTextArea getExpectedValueField() {
        if (expectedValue == null) {
        	expectedValue = JSyntaxTextArea.getInstance(20, 50);
        	expectedValue.setLanguage("text"); //$NON-NLS-1$
        }
