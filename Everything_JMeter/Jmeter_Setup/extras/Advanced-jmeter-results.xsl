		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
					...
					<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']/assertionResult">	
						<xsl:variable name="messagelength" select="string-length(failureMessage)" />
						<xsl:if test="$messagelength > 0">
						<xsl:value-of select="failureMessage" /> in step (<xsl:value-of select="name" />),
						</xsl:if>
			         </xsl:for-each>
