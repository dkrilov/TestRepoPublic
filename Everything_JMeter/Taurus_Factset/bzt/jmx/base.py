...
class JMX(object):
    ...
    def _get_http_request(url, label, method, timeout, body, keepalive, files=(), encoding=None, follow_redirects=True,
        ...
        if isinstance(body, string_types):
            ...
            if body.keys().__contains__('body-data'):
                JMX.__add_body_from_string(args, body['body-data'], proxy)
            else:
        ...
        if parsed_url.query:
            path += "?" + parsed_url.query + parsed_url.fragment
    ...
    @staticmethod
    def _get_json_path_assertion(jsonpath, expected_value, json_validation, expect_null, invert, regexp=False, tname=None):
        """
        :type jsonpath: str
    ...
    @staticmethod
    def _get_xpath_assertion(xpath, validate_xml, ignore_whitespace, use_tolerant_parser, invert, tname=None):
        """
        :type xpath: str
        :type validate_xml: bool
        :type ignore_whitespace: bool
    ...
    @staticmethod
    def _get_script_assertion(script, cacheKey, filename, parameters, scriptLanguage, tname=None):
        """
        :type script: str
        :type cacheKey: bool
        :type filename: str
        :type parameters: str
        :type scriptLanguage: str
        :type tname: str
        :return: lxml.etree.Element
        """
        if tname==None:
            tname = "Script  assertion"
        element = etree.Element("JSR223Assertion",
                                guiclass="TestBeanGUI",
                                testclass="JSR223Assertion",
                                testname=tname)

        element.append(JMX._string_prop("cacheKey", cacheKey))
        element.append(JMX._string_prop("filename", filename))
        element.append(JMX._string_prop("parameters", parameters))
        element.append(JMX._string_prop("script", script))
        element.append(JMX._string_prop("scriptLanguage", scriptLanguage))

        return element

    @staticmethod
    def _get_resp_assertion(field, contains, is_regexp, is_invert, assume_success=False, tname=None):
        """
    ...
    @staticmethod
    def _get_resp_assertion_code(contains, assume_success=False, is_invert=False, tname=None):
        """
        ...
        invert=8
        if tname==None:
            tname = "Assert %s %s" % ("has",
                                    "[" + ", ".join('"' + text_type(x) + '"' for x in contains) + "]")
        element = etree.Element("ResponseAssertion", guiclass="AssertionGui",
                                testclass="ResponseAssertion", testname=tname)
        if is_invert:
            invert=12
        element.append(JMX._string_prop("Assertion.custom_message", ""))
        element.append(JMX._string_prop("Assertion.test_field", "Assertion.response_code"))
        element.append(JMX._bool_prop("Assertion.assume_success", assume_success))
        element.append(JMX.int_prop("Assertion.test_type", invert))
