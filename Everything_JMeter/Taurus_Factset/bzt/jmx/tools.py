...
class JMeterScenarioBuilder(JMX):
    ...
    def __add_assertions(children, req):
        ...
        for idx, assertion in enumerate(assertions):
            ...
            if assertion.keys().__contains__('code'):
                if not isinstance(assertion['code'], list):
                    assertion['code'] = assertion['code'].split(",")
                children.append(JMX._get_resp_assertion_code(
                                                        assertion['code'],
                                                        assertion.get('assume-success', False),assertion.get('is_invert'), assertion.get('name')))
                children.append(etree.Element("hashTree"))
                pass
            else:
                ...
                children.append(JMX._get_resp_assertion(assertion.get("subject", Scenario.FIELD_BODY),
                                                        ...
                                                        assertion.get('assume-success', False), assertion.get('name')))
                children.append(etree.Element("hashTree"))
        ...
        for idx, assertion in enumerate(jpath_assertions):
            ...
            component = JMX._get_json_path_assertion(assertion.get('jsonpath', exc),
                                                     ...
                                                     assertion.get('validate', True),
                                                     assertion.get('expect-null', False),
                                                     assertion.get('invert', False),
                                                     assertion.get('regexp', False), assertion.get('name'))
        ...
        for idx, assertion in enumerate(xpath_assertions):
            ...
            component = JMX._get_xpath_assertion(assertion.get('xpath', exc),
                                                 ...
                                                 assertion.get('invert', False), assertion.get('name'))
            children.append(component)
            children.append(etree.Element("hashTree"))

        script_assertions = req.config.get("assert-script", [])
        for idx, assertion in enumerate(script_assertions):
            exc = TaurusConfigError('Script not found in assertion: %s' % assertion)
            component = JMX._get_script_assertion(assertion.get('script', exc),
                                                 assertion.get('cacheKey', True),
                                                 assertion.get('filename', ''),
                                                 assertion.get('parameters', ''),
                                                 assertion.get('scriptLanguage', 'groovy'), assertion.get('name'))
            children.append(component)
            children.append(etree.Element("hashTree"))

        vars_expressions = req.config.get("vars-expression", [])
        for idx, assertion in enumerate(vars_expressions):
            exc = TaurusConfigError('Token Name Not Found: %s' % assertion)
            component = JMX._get_extractor(assertion.get('expr', exc),
                                                 assertion.get('headers', ''),
                                                 assertion.get('regexp', "\"{0}\":\"(.+?)\"".format(assertion.get('expr',''))),
                                                 assertion.get('template', 1),
                                                 assertion.get('match_no', 1), assertion.get('default', False),
                                                 assertion.get('scope', ''),
                                                 assertion.get('from_var', ''))
            children.append(component)
            children.append(etree.Element("hashTree"))
