...
class SoapUIScriptConverter(object):
    ...
    def _extract_headers(self, config_elem):
        ...
        headers = {entry.get('key'): '${Token}' if entry.get('key').__contains__('X-Fds-Auth') else entry.get('value')
    ...
    def _extract_assertions(self, config_elem):
        ...
        for assertion in assertion_tags:
            ...
            if assertion.get('type') in ('Valid HTTP Status Codes','Invalid HTTP Status Codes'):
                code = assertion.findtext('./con:configuration/codes', namespaces=self.NAMESPACES)
                name = assertion.get('name')
                is_invert = False if assertion.get('type') in ('Valid HTTP Status Codes') else True
                assertions.append({
                                    "name": name,
                                    "code": code,
                                    "is_invert": is_invert
    ...
    def _extract_assertions_json(self, config_elem):
        assertions = []
        assertion_tags = config_elem.findall('.//con:assertion', namespaces=self.NAMESPACES)
        for assertion in assertion_tags:
            if assertion.get('type') in ('JsonPath Existence Match', 'JsonPath Match'):
                path = assertion.findtext('./con:configuration/path', namespaces=self.NAMESPACES)
                content = assertion.findtext('./con:configuration/content', namespaces=self.NAMESPACES)
                allowWildcards =  assertion.findtext('./con:configuration/allowWildcards', namespaces=self.NAMESPACES)
                ignoreNamspaceDifferences =  assertion.findtext('./con:configuration/ignoreNamspaceDifferences', namespaces=self.NAMESPACES)
                ignoreComments =  assertion.findtext('./con:configuration/ignoreComments', namespaces=self.NAMESPACES)
                name = assertion.get('name') 
                assertions.append({
                                    "name": name,
                                    "jsonpath": path,
                                   ...
                                   "ignoreNamspaceDifferences": ignoreNamspaceDifferences,
                                   "ignoreComments": ignoreComments,
                                   })
        return assertions

    def _extract_assertions_xpath(self, config_elem):
        assertions = []
        assertion_tags = config_elem.findall('.//con:assertion', namespaces=self.NAMESPACES)
        for assertion in assertion_tags:
            if assertion.get('type') in ('XPath Match'):
                path = assertion.findtext('./con:configuration/path', namespaces=self.NAMESPACES)
                content = assertion.findtext('./con:configuration/content', namespaces=self.NAMESPACES)
                allowWildcards =  assertion.findtext('./con:configuration/allowWildcards', namespaces=self.NAMESPACES)
                ignoreNamspaceDifferences =  assertion.findtext('./con:configuration/ignoreNamspaceDifferences', namespaces=self.NAMESPACES)
                ...
                assertions.append({
                                    "name": name,
                                    "xpath": path+"="+(content if content else '*'),
                                   "allowWildcards": allowWildcards,
                                   "ignoreNamspaceDifferences": ignoreNamspaceDifferences,
                                   "ignoreComments": ignoreComments,
                                   })
    ...
    def _extract_assertions_script(self, config_elem):
        assertions = []
        assertion_tags = config_elem.findall('.//con:assertion', namespaces=self.NAMESPACES)
        for assertion in assertion_tags:
            if assertion.get('type') in ('GroovyScriptAssertion'):
                scriptText = assertion.findtext('./con:configuration/scriptText', namespaces=self.NAMESPACES)
                name = assertion.get('name') 
                assertions.append({
                                    "name": name,
                                    "script": scriptText
    ...
    def _extract_http_request(self, test_step, vars_list):
        ...
        assertions_json = self._extract_assertions_json(config)
        assertions_xpath = self._extract_assertions_xpath(config)
        assertions_script = self._extract_assertions_script(config)
        
        vars_expression=[]
        for obj in vars_list:
            if obj['Testcase-name'] == test_step.get('name'):
                vars_expression.append(obj)

        if len(vars_expression)!=0:
            request['vars-expression']=vars_expression
        ...
        if assertions_json:
            request["assert-jsonpath"] = assertions_json

        if assertions_xpath:
            request["assert-xpath"] = assertions_xpath

        if assertions_script:
            request["assert-script"] = assertions_script
    ...
    def _calc_base_address(self, test_step):
        end_point=test_step.findtext('./con:config/con:restRequest/con:endpoint',namespaces=self.NAMESPACES)
        if end_point==None:
            config = test_step.find('./con:config', namespaces=self.NAMESPACES)
            ...
            for interface in interfaces:
                if interface.get("name") == service:
                    ...
                    if endpoint is not None:
                        end_point = endpoint.text
                        break
        return end_point
    
    def _search_in_environments(self,servicename):
        environments = self.tree.xpath('//con:environment', namespaces=self.NAMESPACES)
        final_endpoint=None
        for environment in environments:
            service = environment.find('.//con:service[@name="%s"]'%(servicename), namespaces=self.NAMESPACES)
            if service is  not None:
                endpoint=service.find('./con:endpoint',namespaces=self.NAMESPACES)
                if endpoint is not None:
                    final_endpoint=endpoint.text
                break
        return final_endpoint

    def _extract_rest_request(self, test_step, vars_list):
        config = test_step.find('./con:config', namespaces=self.NAMESPACES)
        method = self._parse_parent_formethod(config)
        params = self._parse_parent_resources(config)
        # if( method in  ("POST", "PUT")):
        body_data=self._parse_for_post_body(config)
        end_point=self._calc_base_address(test_step)
        if end_point==None:
            attemp1=self._search_in_environments(config.get('service'))
            if attemp1==None:
                end_point="<Path Not found while converting! Replace this with actual URL>"
            else:
                end_point=attemp1
        url = end_point + config.get('resourcePath')
        if body_data is not None:
            params['body-data']=body_data
            for id,key in enumerate(params.keys()-['body-data']):
                if id==0:
                    url=url+'?'
                url=url+key+'='+params[key]+'&' if len(params[key])!=0 else url
            if url[-1] == '&':
                url=url[:-1]
    ...
    def _parse_parent_resources(self, config):
        ...
        if self.interface is not None:
            for interface in self.interface:
                if interface.get('name')==service_name:
                    method_obj = interface.find('./con:resource/con:method[@name="%s"]' % method_name, namespaces=self.NAMESPACES)
        ...
        if method_obj is not None:
            ...
            # For values inside parameters of the actual request
            for param in method_obj.findall('./con:parameters/con:parameter', namespaces=self.NAMESPACES):
                param_name = param.findtext('./con:name', namespaces=self.NAMESPACES)
                param_value = param.findtext('./con:value', namespaces=self.NAMESPACES)
                def_value = param.findtext('./con:default', namespaces=self.NAMESPACES)
                if param_value:
    ...
    def _parse_parent_formethod(self, config):
        method_name = config.get('methodName')
        service_name=config.get('service')
        method_obj=None
        method=None
        if self.interface is not None:
            for interface in self.interface:
                if interface.get("name")==service_name:
                    method_obj = interface.find('./con:resource/con:method[@name="%s"]' % method_name, namespaces=self.NAMESPACES)
                    if method_obj is not None:
                        method=method_obj.get("method")
                        # break

        return method

    def _parse_for_post_body(self, config):
        body= config.find('./con:restRequest/con:request', namespaces=self.NAMESPACES)
        if body is not None:
            return body.text
    ...
    def _extract_vars(self, config_elem):
        headers_settings = config_elem.find(
            './/con:settings/con:setting[@id="com.eviware.soapui.impl.wsdl.WsdlRequest@request-headers"]',
            namespaces=self.NAMESPACES)
        if headers_settings is None:
        ...
        for entry in entries:
            val = entry.get('value')
            if(val.startswith('${')):
                test_name=self.find_between(entry.get('value'), '${', '#')
                if test_name != None:
                    token=self.find_between_r(entry.get('value'), '/', '}')
                    if token!=None:
                        vars['Testcase-name']=test_name
                        vars['expr']= str(token)
        return vars


    def _extract_scenario(self, test_case, case_level_props, project):
        ...
        for step in steps:
            if step.get("type") == "httprequest":
                config = step.find('./con:config', namespaces=self.NAMESPACES)
                vars = self._extract_vars(config)
                if len(vars.keys())!=0 and vars not in vars_list:
        ...
        for step in steps:
            request = None
            if step.get("type") == "httprequest":
                request = self._extract_http_request(step, vars_list)
            elif step.get("type") == "restrequest":
                request = self._extract_rest_request(step, vars_list)
    ...
    def _extract_test_case(self, test_case, test_suite, suite_level_props, project):
        ...
        scenario = self._extract_scenario(test_case, case_level_props, project)
    ...
    def _extract_config(self, project, test_suites, target_test_case=None):
        ...
        for suite in test_suites:
            ...
            for case in test_cases:
                ...
                scenario_name, scenario = self._extract_test_case(case, suite, suite_props,project)
